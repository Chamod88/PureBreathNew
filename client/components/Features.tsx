  ...
  return (
    <section className="bg-white w-full py-12 md:py-20">
      <div className="max-w-[1440px] mx-auto px-6 md:px-24">
        <h2 className="text-3xl md:text-[36px] font-bold mb-12 md:mb-16">
          Key Features
        </h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8 lg:gap-10">
          <FeatureCard
            ...
            description="capability of an AI system to process input data and deliver insights"
          />
          <FeatureCard
            icon={<AccuracyIcon />}
            title="High Accuracy"
            description="A high percentage of correct predictions"
          />
          <FeatureCard
            icon={<SecurityIcon />}
            title="Secure & Private"
            description="privacy is preserved throughout data collection, processing, storage, and inference"
          />
        </div>
      </div>
    </section>
...
function FeatureCard({ icon, title, description }: FeatureCardProps) {
  return (
    <div className="feature-card bg-white border-[3px] border-[#D9D9D9] rounded-[11px] p-8 shadow-[3px_6px_14px_0_rgba(0,0,0,0.25)] hover:shadow-[3px_8px_18px_0_rgba(0,0,0,0.3)] transition-shadow">
      <div className="mb-6">{icon}</div>
      <h3 className="text-2xl md:text-[32px] font-semibold mb-4">{title}</h3>
      <p className="text-lg md:text-2xl leading-relaxed">{description}</p>
    </div>
  );
}
...
function AIIcon() {
  return (
    <div className="relative w-[72px] h-[67px]">
      <svg
      ...
      <svg
        className="absolute left-[14px] top-[16px]"
        width="21"
        height="24"
        viewBox="0 0 21 24"
        fill="none"
      ...
      >
        <path
          d="M9.73404 0H5.83172L0 24H2.97969L4.32592 18.375H11.0455L12.3569 24H15.4381L9.73404 0ZM4.7176 15.972L7.67118 2.85H7.80464L10.6567 15.972H4.7176ZM18.0986 0H21V24H18.0986V0Z"
          fill="#2683DB"
        />
      </svg>
    </div>
